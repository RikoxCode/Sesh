...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $projects = Project::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (Project $project) use ($years, $users) {
        ...
        $chapters = Chapter::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (Chapter $chapter) use ($projects) {
        ...
        $sections = Section::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (Section $section) use ($chapters) {
        ...
        TextBlock::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (TextBlock $tb) use ($sections) {
        ...
        TableModel::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (TableModel $tbl) use ($sections) {
        ...
        Image::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (Image $img) use ($sections) {
        ...
        AiRating::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (AiRating $r) use ($sections) {
        ...
        Glossary::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (Glossary $g) use ($projects) {
                $g->project_id = $projects->random()->id;
                $g->save();
            });

        // Criteria and SubCriteria linked to Years/Criteria
        $criteria = Criterion::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (Criterion $c) use ($years) {
                $c->year_id = $years->random()->id;
                $c->special_for_project_id = null;
                $c->save();
            });

        $subCriteria = SubCriterion::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (SubCriterion $sc) use ($criteria) {
        ...
        TimeBlock::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (TimeBlock $tb) use ($projects) {
        ...
        UserMeta::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (UserMeta $um) use ($users) {
                $um->user_id = $users->random()->id;
                $um->save();
            });

        UserSubCriterion::factory()
            ->count(rand(5, 10))
            ->make()
            ->each(function (UserSubCriterion $usc) use ($users, $subCriteria) {
